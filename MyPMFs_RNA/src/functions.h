...
std::map<std::string,std::string> create_map(){
    ...
    three2one["A"]="A"; three2one["C"]="C"; three2one["G"]="G"; three2one["U"]="U"; // "one to one" for nucleic acids
    three2one["ADE"]="A"; three2one["CYT"]="C"; three2one["GUA"]="Y"; three2one["URI"]="U"; // for compatibility with obsolete nomenclature
    return three2one;
}
...
std::set<std::string> set_RNAallatom(){
    // The 85 RNA atom types
    std::set<std::string> atypes = {"AC1'", "AC2", "AC2'", "AC3'", "AC4", "AC4'", "AC5", "AC5'", "AC6", "AC8", "AN1", "AN3", "AN6", "AN7", "AN9", "AO2'", "AO3'", "AO4'", "AO5'", "AOP1", "AOP2", "AP", "CC1'", "CC2", "CC2'", "CC3'", "CC4", "CC4'", "CC5", "CC5'", "CC6", "CN1", "CN3", "CN4", "CO2", "CO2'", "CO3'", "CO4'", "CO5'", "COP1", "COP2", "CP", "GC1'", "GC2", "GC2'", "GC3'", "GC4", "GC4'", "GC5", "GC5'", "GC6", "GC8", "GN1", "GN2", "GN3", "GN7", "GN9", "GO2'", "GO3'", "GO4'", "GO5'", "GO6", "GOP1", "GOP2", "GP", "UC1'", "UC2", "UC2'", "UC3'", "UC4", "UC4'", "UC5", "UC5'", "UC6", "UN1", "UN3", "UO2", "UO2'", "UO3'", "UO4", "UO4'", "UO5'", "UOP1", "UOP2", "UP"};
    return atypes;
}


std::set<std::string> set_N1N9(){
    // The 4 RNA N1N9 atom types
    std::set<std::string> atypes = {"AN9", "CN1", "GN9", "UN1"};
    return atypes;
}


std::set<std::string> set_RNAallatomCG(){
    // The 26 coarse-grained RNA atom types
    std::set<std::string> atypes = {"ABB1", "ABB2", "ABB3", "ASC1", "ASC2", "ASC3", "ASC4", "CBB1", "CBB2", "CBB3", "CSC1", "CSC2", "CSC3", "GBB1", "GBB2", "GBB3", "GSC1", "GSC2", "GSC3", "GSC4", "UBB1", "UBB2", "UBB3", "USC1", "USC2", "USC3"};
    return atypes;
}
...
std::vector<std::string> file2vec(std::string filename, std::map<std::string,std::string> three2one, std::set<std::string> atypes){
    ...
    while(getline(fh, line) and line.substr(0,6) != "ENDMDL"){ // will only consider the first MODEL
        if (line.size() > 53 and line.substr(0,4) == "ATOM"){
            ...
            threeletter.erase(std::remove(threeletter.begin(),threeletter.end(),' '),threeletter.end()); // for one-letter residue names (nucleic acids)
